...
//We got some new ideas to find best jets pair
//#include "1L2JAnalysisNewIdeas.h" 
#include "1L2JAnalysisNewIdeas_Sep12022.h"
//2L Analysis
#include "2LAnalysis.h"
...
void VLLAna::SlaveBegin(TTree * /*tree*/)
{
  ...
  n_ll=0,n_lt=0,n_tt=0,n_l2j=0,n_l2j_vetolooselep=0;
  ...
  //Book2LAnalysisHistograms();
  //InitializeMVATreeBranch();

  //LOCAL SETUP
  _Analysis1L2J_HstFile = new TFile("1L2JAnalysis_WJetsInclusive_tree.root","recreate");
  InitializeMVATreeBranch();
  //BookHistograms();
  //Book1L2JAnalysisHistograms();
  //Book2LAnalysisHistograms();
...
void VLLAna::SlaveTerminate()
{
  ...
  cout<<"                                               "<<endl;
  cout<<"             1L2J Channel Analysis             "<<endl;
  cout<<"-----------------------------------------------"<<endl;
  cout<<"No of 1L2J Events veto extra loose lepton= "<<n_l2j_vetolooselep<<endl;
...
Bool_t VLLAna::Process(Long64_t entry)
{
  ...
  if(GoodEvt){
	...
	if(abs(llep.at(0).id)==13 && llep.at(0).v.Pt()>26)              // Muon PT>26 GeV
	  passTrigger=true;
	if(abs(llep.at(0).id)==11 && llep.at(0).v.Pt()>35)              // Ele PT >35 GeV
	  passTrigger=true;
	...
	if(passTrigger){
	  ...
	  else if((int)taus.size()>1){                                  // >2T events//(should trigger on Taus, so it's wrong)
	    is_tt_event=true;n_tt++;                                    // I'm Lazy. 2T is not our interest. Fix somedays.
	  }
	}
      }
	...
	n_l2j_vetolooselep++,h.vllclass->Fill(4);
	
	//Test 1L2JAnalysis.h file
	Analysis();
      }
